...
description: >
  ...
  "아이디어를 앱으로", "IDEA.md", "프로젝트 생성", "프로젝트 빌더", "idea to app",
  "project builder", "앱 만들어줘", "서비스 문서화", "프로젝트 문서", "PRD 생성",
  "아키텍처 문서", "API 명세 작성", "문서 생성", "service docs",
  "project documentation" 등의 키워드로 트리거.
---
...
```
Database 설계 → Backend API → Frontend UI → Testing
```

| 순서 | 담당 | 입력 | 산출물 |
|------|------|------|--------|
| 1 | database | 데이터 모델 요구사항 | schema.sql, models |
| 2 | backend | API 요구사항 + models | API 서버, requirements |
| 3 | frontend | UI 요구사항 + API 엔드포인트 | 프론트엔드 앱 |
| 4 | testing | 전체 코드베이스 | 테스트 코드, 체크리스트 |
